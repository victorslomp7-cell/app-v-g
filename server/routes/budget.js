import { Router } from 'express'
import { randomUUID } from 'crypto'
import { db } from '../db.js'

const categories = Router()
const expenses = Router()

categories.get('/', (req, res) => {
  res.json(db.prepare('SELECT * FROM budget_categories ORDER BY position').all())
})

categories.post('/', (req, res) => {
  const c = req.body
  const id = randomUUID()
  const maxPos = db.prepare('SELECT COALESCE(MAX(position), -1) as m FROM budget_categories').get().m
  db.prepare('INSERT INTO budget_categories (id, name, planned_amount, position) VALUES (?, ?, ?, ?)').run(
    id,
    c.name,
    c.planned_amount || 0,
    maxPos + 1
  )
  res.status(201).json(db.prepare('SELECT * FROM budget_categories WHERE id = ?').get(id))
})

categories.put('/:id', (req, res) => {
  const existing = db.prepare('SELECT * FROM budget_categories WHERE id = ?').get(req.params.id)
  if (!existing) return res.status(404).json({ error: 'Categoria não encontrada' })
  const c = { ...existing, ...req.body, id: req.params.id }
  db.prepare('UPDATE budget_categories SET name=@name, planned_amount=@planned_amount, position=@position WHERE id=@id').run(c)
  res.json(db.prepare('SELECT * FROM budget_categories WHERE id = ?').get(req.params.id))
})

categories.delete('/:id', (req, res) => {
  db.prepare('DELETE FROM budget_categories WHERE id = ?').run(req.params.id)
  res.status(204).end()
})

expenses.get('/', (req, res) => {
  res.json(db.prepare('SELECT * FROM expenses ORDER BY date DESC').all())
})

expenses.post('/', (req, res) => {
  const e = req.body
  const id = randomUUID()
  db.prepare(
    `INSERT INTO expenses (id, category_id, description, amount, status, date, vendor_id)
     VALUES (@id, @category_id, @description, @amount, @status, @date, @vendor_id)`
  ).run({
    id,
    category_id: e.category_id || null,
    description: e.description,
    amount: e.amount || 0,
    status: e.status || 'pendente',
    date: e.date || null,
    vendor_id: e.vendor_id || null,
  })
  res.status(201).json(db.prepare('SELECT * FROM expenses WHERE id = ?').get(id))
})

expenses.put('/:id', (req, res) => {
  const existing = db.prepare('SELECT * FROM expenses WHERE id = ?').get(req.params.id)
  if (!existing) return res.status(404).json({ error: 'Gasto não encontrado' })
  const e = { ...existing, ...req.body, id: req.params.id }
  db.prepare(
    `UPDATE expenses SET category_id=@category_id, description=@description, amount=@amount,
     status=@status, date=@date, vendor_id=@vendor_id WHERE id=@id`
  ).run(e)
  res.json(db.prepare('SELECT * FROM expenses WHERE id = ?').get(req.params.id))
})

expenses.delete('/:id', (req, res) => {
  db.prepare('DELETE FROM expenses WHERE id = ?').run(req.params.id)
  res.status(204).end()
})

export default { categories, expenses }
